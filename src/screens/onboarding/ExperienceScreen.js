...
const ExperienceScreen = ({ navigation }) => {
  ...
  const options = [
    { id: 'yes', label: 'Yes', icon: <ThumbsUp size={24} color={colors.text} /> },
    { id: 'no', label: 'No', icon: <ThumbsDown size={24} color={colors.text} /> },
  ];

  return (
    <SafeAreaView style={styles.container}>
      <Header onBack={() => navigation.goBack()} progress={20} />
